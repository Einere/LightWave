...
# 19/04/11
## RecyclerView에 올려진 이미지 제거 기능 구현 (CHJ)
RecyclerView에 올려진 이미지를 롱탭시 제거하는 기능 구현.
  
## 사진 선택시, 텍스트 파일 복사하는 기능 삭제 (CHJ)
불필요한 기능이므로 삭제.
  
## 소켓 연결상태를 확인하는 스레드 실행 함수 추가 (CHJ)
소켓의 연결상태를 주기적으로 확인하며, 끊어져있는 경우 재연결을 시도하는 스레드를 실행하는 함수인 checkStatus를 추가.

## 서버 작업 삭제 기능 추가 (KJW)
작업 삭제시 해당 작업 관련 모든 파일을 삭제한다.

## 서버 작업관리 창 상호작용 추가 (KJW)
특정 작업에 대해 우클릭 메뉴를 통해 활성화, 삭제와 같은 작업 관리를 진행할 수 있다.

## 서버 작업필지 등록 기능 추가 (KJW)
작업도면의 우클릭 메뉴를 통해 선택한 점들을 작업필지로 등록할 수 있다.

## 서버 파일 저장 / 로드 구현 (KJW)
로컬에 파일을 저장하고 로드하기 위한 인터페이스 정의 및 구현

## 서버 요청으로 전달된 촬영 이미지 저장 기능 구현 (KJW)
base64 디코딩 이후 jpeg 형태로 저장
  
## 서버로부터 패킷을 받는 기능 확인 (KJW, CHJ)
서버로부터 json형식의 String형 응답 패킷을 정상적으로 받는 것을 확인.  
- 작업자 이름으로 요청을 했을 때, 확인용 응답 패킷을 확인.
- 작업 정보를 요청을 했을 때, 작업을 시작하지 않은 경우, 작업 정보 응답 패킷을 확인.
- 작업 정보를 요청을 했을 때, 작업을 시작한 경우, 작업 정보 응답 패킷을 확인.
  
# 19/04/24

## 요청에 대한 처리결과를 나타내는 응답코드(status) 추가 (KJW)
2 - 정상처리, 4 - 비정상처리

## 촬영정보 저장 기능 보완 (KJW)
사진과 메모뿐 아니라 작업자와 업데이트 시간까지 기록할 수 있다.

## 지도위에 측량점 표시 (KJW)
동그랗게
  
## 연결 시 요청 패킷 수정 (CHJ)
ConnectionActivity에서, 서버에 연결시 보내는 패킷을 수정.

## 패킷명세서 수정 (KJW, CHJ)
"초기화 및 권한 부여"패킷 일부 수정.  
"Task"수정 및 "SurveyPoint"추가.
